...
hparams1 = tf.contrib.training.HParams(
	...
	max_time_steps = 8000, #Max time steps in audio used to train wavenet (decrease to save memory) (Recommend: 8000 on modest GPUs, 13000 on stronger ones)
	###########################################################################################################################################
	...
	tacotron_batch_size = 10, #number of training samples on each training steps
	...
	wavenet_test_size = 0.0960, #% of data to keep as test data, if None, wavenet_test_batches must be not None
	...
	# 'Chúng thủy tàn phá môi trường',
	# 'tuy nhiên kết quả lại không như mong đợi',
	# 'mình tên là dũng',
	# 'mình sống tại hà nội',
	# 'sinh ra và lớn lên ở quảng ninh',
	# 'anh thấy em nói hay không',
	# 'a hi hi',
	# 'anh Đạt không đi ăn à',
	# 'sao lại ngủ luôn rồi',
	# 'Mặc dù thiếu bằng chứng về cái gọi là phần cứng gián điệp',
	'Theo đó các nhân viên điều tra phát hiện ra rằng những con chíp siêu nhỏ không to hơn hạt cát là mấy được các nhà thầu phụ của Súp ở Trung Quốc cài vào trong quá trình lắp ráp các thiết bị',
	# 'nhưng lại băn khoăn về nhiều vấn đề khác như'
	]
	)
